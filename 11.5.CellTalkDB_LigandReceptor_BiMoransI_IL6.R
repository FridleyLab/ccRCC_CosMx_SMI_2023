...
rm(list=ls()) #no idea why remove again to then be imported below but whatever
...
IL6_genes = intersect(msigdbr_list$HALLMARK_IL6_JAK_STAT3_SIGNALING, colnames(scaled_expression))
IL6_lr_pairs = ligand_pairs_available %>%
  filter(ligand_gene_symbol %in% IL6_genes, receptor_gene_symbol %in% IL6_genes)
...
morans_list = pbmclapply(setNames(fovs, fovs), function(fov){
  ...
  df = lapply(IL6_lr_pairs$lr_pair, function(pair){
    global_moran = moran_bv(spdf[[IL6_lr_pairs$ligand_gene_symbol[IL6_lr_pairs$lr_pair == pair]]], 
                            spdf[[IL6_lr_pairs$receptor_gene_symbol[IL6_lr_pairs$lr_pair == pair]]], knn.nb.listw, nsim = 100)
...
#saveRDS(morans_list, "Manley_SMI/results/ligand_receptor/Manley_ligand_receptor_CellTalkDB-knn3_IL6.rds")
morans_list = readRDS("Manley_SMI/results/ligand_receptor/Manley_ligand_receptor_CellTalkDB-knn3_IL6.rds")

#Making data frame of data
morans_df = do.call(bind_rows, morans_list) %>%
  spread("lr_pair", "original")
lr_pairs=IL6_lr_pairs$lr_pair
...
saveWorkbook(wb, "Manley_SMI/results/ligand_receptor/Primary_Sarcomatoid_ligand-receptor_bivarMoransI_knn3_IL6.xlsx", overwrite = TRUE)
